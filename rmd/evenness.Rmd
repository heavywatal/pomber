---
title: "Motif occurrence in sliding windows in S. pombe"
output:
  html_document:
    css: style.css
runtime: shiny
...
```{r data, echo=FALSE}
#.cache = '~/git/pomber/rmd/data/vm-ratio.csv.gz'
.cache = 'data/vm-ratio.csv.gz'
if (file.exists(.cache)) {
    .data = read.table(.cache, header=TRUE, stringsAsFactors=FALSE)
} else {
...
if (FALSE) {
    ...
    .data %>>%
        group_by(chr) %>>%
        summarise(var=var(observed), mean=mean(observed), vm_ratio=var/mean)
}
...
renderDataTable(filtered(), options=list(order=c(4, 'asc')))
```
